<?php
	...
	{
		$mid = $_GET['mid'];
		$query = $pdo->prepare('DELETE FROM lp_subjects WHERE id = ?');
			$query->bindValue(1, $mid);
			$query->execute();
			
		$query = $pdo->prepare('DELETE FROM lp_aims WHERE lp_subid = ?');
			$query->bindValue(1, $mid);
			$query->execute();
			
		$query = $pdo->prepare('DELETE FROM lp_newmethods WHERE lp_subid = ?');
			$query->bindValue(1, $mid);
			$query->execute();
		
		header('Location: lpsubjectsview.php');
	}
?>
<html>
	<head>
					...
					<div class="accordion" id="accordion-1995">
						...
						?>		<div class="accordion-group">
									<div class="accordion-heading">
										<a class="last btn btn-danger" href="lpsubjectsview.php?mid=<?php echo $article['id'];?>" >Delete</a>
										<a class="accordion-toggle" href="lpinsubject.php?id=<?php echo $article['id']; ?>"><?php echo $article['lpsubject'];?></a>
									</div>
								</div>
								<?php $i++;
							}?>
