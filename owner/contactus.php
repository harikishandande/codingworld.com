...
if(isset($_SESSION['owner'])) {
        if(isset($_GET['id'])) {
                $id = $_GET['id'];
                $query = $pdo->prepare('DELETE FROM contactus WHERE id = ?');
		...
		<div class="container-fluid">
			<div class="row-fluid">
					...
					<?php
						...
						include_once('../includes/article.php');
						$user = new Article;
						$users = $user->fetch_contactus();
						foreach($users as $user)
						{	static $i=1;
					?>
								<div class="accordion-group">
									<div class="accordion-heading">
										<a class="last btn btn-danger" href="contactus.php?id=<?php echo $user['id'];?>" >Delete</a>
								...
								<?php
									...
									include_once('../includes/article.php');
									$article = new Article;
									$articles = $article->fetch_contact($guest);
								...
								?>
									<div id="accordion-element-<?php echo $i;?>" class="accordion-body collapse">
										<div class="accordion-inner">
														...
														<h4>Message : <?php echo $article['message']; ?></h4>
										</div>
									</div>
									<?php }?>
								</div>
